...
interface HotspotSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const HotspotSelectionModal: React.FC<HotspotSelectionModalProps> = ({
  isOpen,
  onClose,
  onSelect,
  areaId,
  ...
  useEffect(() => {
    if (currentHotspotId) {
      setSelectedHotspotId(currentHotspotId);
    }
  }, [currentHotspotId]);
  ...
  const fetchHotspots = async () => {
    setIsLoading(true);
    try {
      const data = await getHotspotsByAreaId(areaId.toString());
      setHotspots(data);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[800px] max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Chọn địa điểm chính</DialogTitle>
          <DialogDescription>
            Chọn địa điểm sẽ được đặt làm địa điểm chính cho khu vực này
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-gray-500">
              ...
              <p className="text-sm">
                {searchTerm
                  ...
                  : "Hãy tạo địa điểm mới cho khu vực này"}
              </p>
            </div>
          )}
        ...
        <DialogFooter className="flex-shrink-0">
          <div className="flex space-x-2">
            ...
            >
              {selectedHotspotId ? "Xác nhận chọn" : "Chọn một địa điểm"}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
...
export default HotspotSelectionModal;
